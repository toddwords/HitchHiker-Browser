...
function bindGuideActions(){
	...
	$('#goDashboard').click(function(){
		ipcRenderer.send("makeDashboard", {})
	})
	...
	$('#speakChat').change(function(){
		...
		store.set({speakChat:$(this).prop("checked")})
	})
	...
	$('#scrollSync').change(function(){
		...
		store.set({scrollSync:$(this).prop("checked")})
	})
	...
	$('#recordActions').change(function(){
		...
		store.set({isRecording:$(this).prop("checked")})
	})
...
function newPage(newURL){
	ipcRenderer.send('newPageClient', {newPageClient:newURL})
}

function addWebsite(){
	USER.performances[USER.currentPerformance].urlList.push(USER.currentURL)
